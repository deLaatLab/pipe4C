...
frag.MP<-function( vpChr_FragMap=NULL, firstcutter_FragMap="GATC", secondcutter_FragMap="GTAC", genome_FragMap="hg19", captureLen_FragMap=60, nThreads_FragMap=10, baseFolder_FragMap, Bowtie2Folder, config_genomes ) {
  ...
  fragsGR2 <- fragsGR[, c("pos", "type", "fe_strand", "fe_id", paste0( "len", captureLen_FragMap ) ) ]
  colnames( elementMetadata( fragsGR2 ) )[5] <- "unique"
  fragsGR <- fragsGR[ fragsGR$unique ]
  }else{
  
  message('         ### Not hg19 NlaIII DpnII')  
  
    fragsGR <- getFragMap(
      vpChr_FragMap=NULL
      ,firstcutter_FragMap=firstcutter_FragMap
      ,secondcutter_FragMap=secondcutter_FragMap
      ,genome_FragMap=genome_FragMap
      ,captureLen_FragMap=captureLen_FragMap
      ,nThreads_FragMap=nThreads_FragMap
      ,baseFolder_FragMap=baseFolder_FragMap
      ,Bowtie2Folder=Bowtie2Folder
      ,config_genomes=config_genomes
    )  
...
Run.4Cpipeline <- function( VPinfo.file, FASTQ.F, OUTPUT.F, configuration){
  ...
  fragMP <- "/data2/repository/genome/fragmap_4C/PK/MP/hg19_GTAC_GATC.rds"
  fragMP.RDS<-readRDS(fragMP)
  ...
  for ( i in 1:length( exp.name ) ) {
    ...
    if ( bins == TRUE | make.gwplot == TRUE) {
      if ( analysis[i] == "all" ) {
        ...
        bin.GR <- make.reads.and.bins( reads=readsAln, assemblyName=genome[i], res=configuration$binSize, config_genomes=configuration$genomes )
...
genomes <- data.frame( 
  genome=as.character(c( "hg19","mm9" ))
  , BSgenome=as.character(c( "BSgenome.Hsapiens.UCSC.hg19","BSgenome.Hsapiens.UCSC.mm9" ))
  , stringsAsFactors=FALSE 
  , row.names=1
)
...
getFragMap(
  vpChr_FragMap=NULL
  ,firstcutter_FragMap="GTAC"
  ,secondcutter_FragMap="GATC"
  ,genome_FragMap="hg19"
  ,captureLen_FragMap=i
  ,nThreads_FragMap=8
  ,baseFolder_FragMap="/data2/repository/genome/fragmap_4C/PK/MP/"
  ,Bowtie2Folder="/data2/repository/genome/hg19/Bowtie2Index/"
  ,config_genomes=genomes
)
